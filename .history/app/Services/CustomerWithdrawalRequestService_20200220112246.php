...
class CustomerWithdrawalRequestService
{
     ...
     */
    private function processLoanRepaymentTransaction(User $user, Transaction $transaction, string $type, ?string $message)
    {
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $loan_id, array $transactionDetails)
    {
        ...
        $transaction = $this->transactionService->initiateCustomerWithdrawalRequestRepaymentTransaction($loan, $transactionDetails);
