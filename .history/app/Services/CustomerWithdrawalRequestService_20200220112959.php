...
class CustomerWithdrawalRequestService
{
     ...
     */
    public function __construct(CustomerWithdrawalRequestRepositoryInterface $customerwithdrawalrequestRepository, TransactionService $customerwithdrawalrequestService, UserRepositoryInterface $userRepository)
    {
        $this->customerwithdrawalrequestRepository = $customerwithdrawalrequestRepository;
        $this->transactionService = $customerwithdrawalrequestService;
     ...
     *
     * @param User $user
     * @param Transaction $customerwithdrawalrequest
     * @param string $type
     * @param null|string $message
     * @return Transaction
     */
    private function processBranchExtraFundPaymentRequest(User $user, Transaction $customerwithdrawalrequest, string $type, ?string $message)
    {
        DB::transaction(function () use ($customerwithdrawalrequest, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $contributionPlan = $this->contributionRepository->find($customerwithdrawalrequest->owner_id);
                    $this->contributionRepository->withdraw($contributionPlan, $customerwithdrawalrequest);
                    $this->transactionRepository->updateTransactionStatus($customerwithdrawalrequest, TransactionStatus::COMPLETED);
                    break;

                case TransactionProcessingActions::DISAPPROVE:
                    $this->transactionRepository->updateTransactionStatus($customerwithdrawalrequest, TransactionStatus::FAILED);
                    break;
            }

            $this->transactionRepository->storeProcessedTransaction($customerwithdrawalrequest, $user->id, $type, $message);
        });
     ...
     */
    private function processcustomerwithdrawalrequestRepaymentTransaction(User $user, Transaction $customerwithdrawalrequest, string $type, ?string $message)
    {
        DB::transaction(function () use ($customerwithdrawalrequest, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($customerwithdrawalrequest->owner_id);
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, $customerwithdrawalrequest->request_amount);
                    $this->transactionRepository->updateTransactionStatus($customerwithdrawalrequest, TransactionStatus::COMPLETED);
                    break;
    ...
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $customerwithdrawalrequestDetails)
    {
        ...
        if ($customerwithdrawalrequestDetails['request_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
        ...
        $customerwithdrawalrequest = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, $customerwithdrawalrequestDetails);
