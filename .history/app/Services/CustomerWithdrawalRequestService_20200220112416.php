...
class CustomerWithdrawalRequestService
{
        ...
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    ...
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, $transaction->transaction_amount);
                    $this->transactionRepository->updateTransactionStatus($transaction, TransactionStatus::COMPLETED);
                    break;
    ...
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $transactionDetails)
    {
        ...
        $re = $transactionDetails['transaction_amount'];

        if ($transactionDetails['transaction_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
            throw new GraphqlError("Transaction amount {$re} is greater than the total customerwithdrawalrequest balance");
        }

        if ($transactionDetails['transaction_type'] !== TransactionType::customerwithdrawalrequest_REPAYMENT) {
