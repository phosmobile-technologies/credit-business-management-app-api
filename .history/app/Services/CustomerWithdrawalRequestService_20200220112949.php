...
class CustomerWithdrawalRequestService
{
     ...
     */
    public function __construct(CustomerWithdrawalRequestRepositoryInterface $customerwithdrawalrequestRepository, TransactionService Service, UserRepositoryInterface $userRepository)
    {
    ...
    public function initiateCustomerWithdrawalRequest(string $owner_id, array Details)
    {
     ...
     * @return Transaction
     */
    private function processBranchExtraFundPaymentRequest(User $user, Transaction , string $type, ?string $message)
    {
        DB::transaction(function () use (, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $contributionPlan = $this->contributionRepository->find(->owner_id);
                    $this->contributionRepository->withdraw($contributionPlan, );
                    $this->transactionRepository->updateTransactionStatus(, TransactionStatus::COMPLETED);
                    break;

                case TransactionProcessingActions::DISAPPROVE:
                    $this->transactionRepository->updateTransactionStatus(, TransactionStatus::FAILED);
                    break;
            }

            $this->transactionRepository->storeProcessedTransaction(, $user->id, $type, $message);
        });
     ...
     */
    private function processcustomerwithdrawalrequestRepaymentTransaction(User $user, Transaction , string $type, ?string $message)
    {
        DB::transaction(function () use (, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find(->owner_id);
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, ->request_amount);
                    $this->transactionRepository->updateTransactionStatus(, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array Details)
    {
        $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($customerwithdrawalrequest_id);

        $requestAmount = Details['request_amount'];

        if (Details['request_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
            throw new GraphqlError("Transaction amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if (Details['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

         = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, Details);
