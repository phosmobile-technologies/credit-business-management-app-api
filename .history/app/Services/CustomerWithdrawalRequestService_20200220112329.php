...
class CustomerWithdrawalRequestService
{
     ...
     */
    private function processcusRepaymentTransaction(User $user, Transaction $transaction, string $type, ?string $message)
    {
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $cus = $this->cusRepository->find($transaction->owner_id);
                    $this->cusRepository->repaycus($cus, $transaction->transaction_amount);
                    $this->transactionRepository->updateTransactionStatus($transaction, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $cus_id, array $transactionDetails)
    {
        $cus = $this->cusRepository->find($cus_id);

        $transactionAmount = $transactionDetails['transaction_amount'];

        if ($transactionDetails['transaction_amount'] > $cus->cus_balance) {
            throw new GraphqlError("Transaction amount {$transactionAmount} is greater than the total cus balance");
        }

        if ($transactionDetails['transaction_type'] !== TransactionType::cus_REPAYMENT) {
            throw new GraphqlError("The transaction type selected must be cus Repayment");
        }

        $transaction = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($cus, $transactionDetails);
