...
class CustomerWithdrawalRequestService
{
     ...
     */
    private function processcustoRepaymentTransaction(User $user, Transaction $transaction, string $type, ?string $message)
    {
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $custo = $this->custoRepository->find($transaction->owner_id);
                    $this->custoRepository->repaycusto($custo, $transaction->transaction_amount);
                    $this->transactionRepository->updateTransactionStatus($transaction, TransactionStatus::COMPLETED);
                    break;
     ...
     * Repay a custo
     *
     * @param string $custo_id
     * @param array $transactionDetails
     * @return \App\Models\Transaction
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $custo_id, array $transactionDetails)
    {
        $custo = $this->custoRepository->find($custo_id);

        $transactionAmount = $transactionDetails['transaction_amount'];

        if ($transactionDetails['transaction_amount'] > $custo->custo_balance) {
            throw new GraphqlError("Transaction amount {$transactionAmount} is greater than the total custo balance");
        }

        if ($transactionDetails['transaction_type'] !== TransactionType::custo_REPAYMENT) {
            throw new GraphqlError("The transaction type selected must be custo Repayment");
        }

        $transaction = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($custo, $transactionDetails);
