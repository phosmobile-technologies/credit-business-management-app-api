...
class CustomerWithdrawalRequestService
{
     ...
     * @param TransactionService $custoService
     * @param UserRepositoryInterface $userRepository
     */
    public function __construct(CustomerWithdrawalRequestRepositoryInterface $customerwithdrawalrequestRepository, TransactionService $custoService, UserRepositoryInterface $userRepository)
    {
        $this->customerwithdrawalrequestRepository = $customerwithdrawalrequestRepository;
        $this->transactionService = $custoService;
        $this->userRepository = $userRepository;
    }

    public function initiateCustomerWithdrawalRequest(string $owner_id, array $custoDetails)
    {
        switch ($custoDetails['request_type']) {
     ...
     *
     * @param User $user
     * @param Transaction $custo
     * @param string $type
     * @param null|string $message
     * @return Transaction
     */
    private function processBranchExtraFundPaymentRequest(User $user, Transaction $custo, string $type, ?string $message)
    {
        DB::transaction(function () use ($custo, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $contributionPlan = $this->contributionRepository->find($custo->owner_id);
                    $this->contributionRepository->withdraw($contributionPlan, $custo);
                    $this->transactionRepository->updateTransactionStatus($custo, TransactionStatus::COMPLETED);
                    break;

                case TransactionProcessingActions::DISAPPROVE:
                    $this->transactionRepository->updateTransactionStatus($custo, TransactionStatus::FAILED);
                    break;
            }

            $this->transactionRepository->storeProcessedTransaction($custo, $user->id, $type, $message);
        });
     ...
     */
    private function processcustomerwithdrawalrequestRepaymentTransaction(User $user, Transaction $custo, string $type, ?string $message)
    {
        DB::transaction(function () use ($custo, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($custo->owner_id);
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, $custo->request_amount);
                    $this->transactionRepository->updateTransactionStatus($custo, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $custoDetails)
    {
        $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($customerwithdrawalrequest_id);

        $requestAmount = $custoDetails['request_amount'];

        if ($custoDetails['request_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
            throw new GraphqlError("Transaction amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if ($custoDetails['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

        $custo = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, $custoDetails);
