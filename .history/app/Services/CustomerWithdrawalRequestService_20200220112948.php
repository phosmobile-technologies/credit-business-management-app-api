...
class CustomerWithdrawalRequestService
{
     ...
     * @param TransactionService cuService
     * @param UserRepositoryInterface $userRepository
     */
    public function __construct(CustomerWithdrawalRequestRepositoryInterface $customerwithdrawalrequestRepository, TransactionService cuService, UserRepositoryInterface $userRepository)
    {
        $this->customerwithdrawalrequestRepository = $customerwithdrawalrequestRepository;
        $this->transactionService = cuService;
        $this->userRepository = $userRepository;
    }

    public function initiateCustomerWithdrawalRequest(string $owner_id, array cuDetails)
    {
        switch (cuDetails['request_type']) {
     ...
     *
     * @param User $user
     * @param Transaction cu
     * @param string $type
     * @param null|string $message
     * @return Transaction
     */
    private function processBranchExtraFundPaymentRequest(User $user, Transaction cu, string $type, ?string $message)
    {
        DB::transaction(function () use (cu, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $contributionPlan = $this->contributionRepository->find(cu->owner_id);
                    $this->contributionRepository->withdraw($contributionPlan, cu);
                    $this->transactionRepository->updateTransactionStatus(cu, TransactionStatus::COMPLETED);
                    break;

                case TransactionProcessingActions::DISAPPROVE:
                    $this->transactionRepository->updateTransactionStatus(cu, TransactionStatus::FAILED);
                    break;
            }

            $this->transactionRepository->storeProcessedTransaction(cu, $user->id, $type, $message);
        });
     ...
     */
    private function processcustomerwithdrawalrequestRepaymentTransaction(User $user, Transaction cu, string $type, ?string $message)
    {
        DB::transaction(function () use (cu, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find(cu->owner_id);
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, cu->request_amount);
                    $this->transactionRepository->updateTransactionStatus(cu, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array cuDetails)
    {
        $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($customerwithdrawalrequest_id);

        $requestAmount = cuDetails['request_amount'];

        if (cuDetails['request_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
            throw new GraphqlError("Transaction amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if (cuDetails['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

        cu = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, cuDetails);
