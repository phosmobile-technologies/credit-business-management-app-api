...
class CustomerWithdrawalRequestService
{
     ...
     * @param TransactionService $cusService
     * @param UserRepositoryInterface $userRepository
     */
    public function __construct(CustomerWithdrawalRequestRepositoryInterface $customerwithdrawalrequestRepository, TransactionService $cusService, UserRepositoryInterface $userRepository)
    {
        $this->customerwithdrawalrequestRepository = $customerwithdrawalrequestRepository;
        $this->transactionService = $cusService;
        $this->userRepository = $userRepository;
    }

    public function initiateCustomerWithdrawalRequest(string $owner_id, array $cusDetails)
    {
        switch ($cusDetails['request_type']) {
     ...
     *
     * @param User $user
     * @param Transaction $cus
     * @param string $type
     * @param null|string $message
     * @return Transaction
     */
    private function processBranchExtraFundPaymentRequest(User $user, Transaction $cus, string $type, ?string $message)
    {
        DB::transaction(function () use ($cus, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $contributionPlan = $this->contributionRepository->find($cus->owner_id);
                    $this->contributionRepository->withdraw($contributionPlan, $cus);
                    $this->transactionRepository->updateTransactionStatus($cus, TransactionStatus::COMPLETED);
                    break;

                case TransactionProcessingActions::DISAPPROVE:
                    $this->transactionRepository->updateTransactionStatus($cus, TransactionStatus::FAILED);
                    break;
            }

            $this->transactionRepository->storeProcessedTransaction($cus, $user->id, $type, $message);
        });
     ...
     */
    private function processcustomerwithdrawalrequestRepaymentTransaction(User $user, Transaction $cus, string $type, ?string $message)
    {
        DB::transaction(function () use ($cus, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($cus->owner_id);
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, $cus->request_amount);
                    $this->transactionRepository->updateTransactionStatus($cus, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $cusDetails)
    {
        $customerwithdrawalrequest = $this->customerwithdrawalrequestRepository->find($customerwithdrawalrequest_id);

        $requestAmount = $cusDetails['request_amount'];

        if ($cusDetails['request_amount'] > $customerwithdrawalrequest->customerwithdrawalrequest_balance) {
            throw new GraphqlError("Transaction amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if ($cusDetails['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

        $cus = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, $cusDetails);
