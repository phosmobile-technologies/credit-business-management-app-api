...
class CustomerWithdrawalRequestService
{
     ...
     */
    private function processcustomerwithdrawaRepaymentTransaction(User $user, Transaction $transaction, string $type, ?string $message)
    {
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    $customerwithdrawa = $this->customerwithdrawaRepository->find($transaction->owner_id);
                    $this->customerwithdrawaRepository->repaycustomerwithdrawa($customerwithdrawa, $transaction->transaction_amount);
                    $this->transactionRepository->updateTransactionStatus($transaction, TransactionStatus::COMPLETED);
                    break;
    ...
    /**
     * Repay a customerwithdrawa
     *
     * @param string $customerwithdrawa_id
     * @param array $transactionDetails
     * @return \App\Models\Transaction
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawa_id, array $transactionDetails)
    {
        $customerwithdrawa = $this->customerwithdrawaRepository->find($customerwithdrawa_id);

        $transactionAmount = $transactionDetails['transaction_amount'];

        if ($transactionDetails['transaction_amount'] > $customerwithdrawa->customerwithdrawa_balance) {
            throw new GraphqlError("Transaction amount {$transactionAmount} is greater than the total customerwithdrawa balance");
        }

        if ($transactionDetails['transaction_type'] !== TransactionType::customerwithdrawa_REPAYMENT) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawa Repayment");
        }

        $transaction = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawa, $transactionDetails);
