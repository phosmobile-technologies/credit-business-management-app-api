...
class CustomerWithdrawalRequestService
{
        ...
        DB::customerwithdrawalrequest(function () use ($customerwithdrawalrequest, $type, $user, $message) {
            switch ($type) {
                case RequestStatus::APPROVED_BY_BRANCH_MANAGER:
                    ...
                    $this->customerwithdrawalrequestRepository->addPayment($customerwithdrawalrequest, $customerwithdrawalrequest);
                    $this->transactionRepository->updateRequestStatus($customerwithdrawalrequest, RequestStatus::APPROVED_BY_BRANCH_MANAGER);
                    break;
    ...
    private function processBranchExtraFundPayentRequest(User $user, CustomerWithdrawalRequest $customerwithdrawalrequest, string $type, ?string $message)
    {
        DB::customerwithdrawalrequest(function () use ($customerwithdrawalrequest, $type, $user, $message) {
            ...
            $this->transactionRepository->storeRequestStatus($customerwithdrawalrequest, $user->id, $type, $message);
        });
    ...
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $transactionDetails)
    {
        ...
        $requestAmount = $transactionDetails['request_amount'];

        if ($transactionDetails['request_amount'] > $customerwithdrawalrequest->request_balance) {
            throw new GraphqlError("Request amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if ($transactionDetails['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

        $customerwithdrawalrequest = $this->customerwithdrawalrequestService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, $transactionDetails);
