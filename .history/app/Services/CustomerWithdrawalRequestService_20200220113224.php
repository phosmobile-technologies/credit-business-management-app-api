...
class CustomerWithdrawalRequestService
{
    ...
    public function initiateCustomerWithdrawalRequest(string $owner_id, array $)
    {
        switch ($['request_type']) {
        ...
        DB::transaction(function () use ($transaction, $type, $user, $message) {
            switch ($type) {
                case TransactionProcessingActions::APPROVE:
                    ...
                    $this->customerwithdrawalrequestRepository->repaycustomerwithdrawalrequest($customerwithdrawalrequest, $transaction->request_amount);
                    $this->transactionRepository->updateTransactionStatus($transaction, TransactionStatus::COMPLETED);
                    break;
     ...
     * @throws GraphqlError
     */
    public function initiateCustomerWithdrawalRequestRepayment(string $customerwithdrawalrequest_id, array $)
    {
        ...
        if ($['request_amount'] > $customerwithdrawalrequest->request_balance) {
            throw new GraphqlError("Request amount {$requestAmount} is greater than the total customerwithdrawalrequest balance");
        }

        if ($['request_type'] !== RequestType::CONTRIBUTION_WITHDRAWAL) {
            throw new GraphqlError("The transaction type selected must be customerwithdrawalrequest Repayment");
        }

        $transaction = $this->transactionService->initiateCustomerWithdrawalRequestRepayment($customerwithdrawalrequest, $);
